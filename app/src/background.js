...
import {
  DayRequest, ListenRequest,
  Settings, SettingsPlusOptionsRequest
...
function createTrayAndMenusAndShortcuts (
  ...
  let toggleCapture = function (val) {
    getSettings().then((settingsOptions) => {
...
app.on('ready', () => {
  ...
  if (require.main && require.main && require.main.filename) {
    isPackaged = require.main.filename.indexOf('app.asar') !== -1
  }

  let binariesPath =
    isProduction && isPackaged
      ? path.join(path.dirname(app.getAppPath()), '..', './Resources', './bin')
      : path.join(root, './relapse', './app', './bin')
  let execPath = path.resolve(path.join(binariesPath, './daemon'))
  let capturePath = app.getPath('documents') + '/RelapseScreenshots/'
  let userDataPath = app.getPath('userData') + '/'

  console.log('root: ', root)
  console.log('bin: ', execPath)
...
function toggleCaptures (isEnabled) {
  if (isEnabled) {
    if (stream) {
...
function loadDay (date, skipToEnd) {
...
function sendDayInfoToApp (resp, date, skipToEnd) {
  // let dayInfo: DayInfo = {
...
function setSettings (settings) {
  return new Promise((resolve, reject) => {
    ...
    client.setSettings(req, (err, response) => {
      if (err != null) {
        console.log('error')
        reject(err)
      }
