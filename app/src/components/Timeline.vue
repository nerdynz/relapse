<template>
  <div class="timeline" :style='timelineReadyStyle'>
    <div class='picker'>
      ...
      <datepicker :value="day" @selected="dayChanged" :highlighted="highlighted" @opened="datePickerOpened"></datepicker>
      <button class="side-btn next-day" @click='nextDay'>
        <fa-icon icon="caret-right" />
      </button>
    </div>
    <div class="canvas-bg" :style='timelineReadyStyle'>
      <button class="side-btn prev-minute" @mousedown='prevMinute' @mouseup='prevMinuteOff'>
        <fa-icon icon="caret-left" />
      </button>
      <canvas ref="timelineCanvas" id="timeline-viewer" width="1280" height="40"></canvas>
      <button class="side-btn next-minute" @mousedown='nextMinute' @mouseup='nextMinuteOff'>
        <fa-icon icon="caret-right" />
      </button>
    ...
    watch: {
      ...
      'minuteChangeIncrement': function () {
        ...
        var changeMinute = function () {
          if (self.minuteChangeIncrement !== 0) { // we have the mouse down
