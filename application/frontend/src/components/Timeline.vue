<template>
  <div class="timeline" :style="timelineReadyStyle">
    ...
    <div class="canvas-bg" :style="timelineReadyStyle">
      <canvas ref="timelineCanvas" id="timeline-viewer" height="40" />
...
import { Capture, CapturedDay } from "../../bindings/github.com/nerdynz/relapse/proto/models";
...
const props = defineProps<{
  ...
  isFollowing: boolean,
  currentDay: CapturedDay
}>()

const times: Capture[] = $computed(() => {
  return (relapseStore.captures) as Capture[]
})
...
const hhmm = $computed(() => {
  if (props.currentDay) {
    let capturedMins = props.currentDay.Summary?.TotalCapturedMinutes ?? 0
