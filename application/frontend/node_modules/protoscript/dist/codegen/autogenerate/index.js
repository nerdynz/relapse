    ...
    types.forEach((node) => {
        ...
        switch (node.type) {
           ...
           */
            ...
            return ${node.content.namespacedName}._writeMessage(msg, new protoscript.BinaryWriter()).getResultBuffer();`;
                    }
                    result += "},\n\n";
                    // decode (protobuf)
                    result += `\
          ...
          `;
            ...
            return ${node.content.namespacedName}._readMessage(${node.content.namespacedName}.initialize(), new protoscript.BinaryReader(bytes));`;
                    }
                    result += "},\n\n";
                    // initialize
                    result += `\
           ...
           */
          initialize: function(msg${printIfTypescript(`?: Partial<${node.content.namespacedName}>`)}) ${printIfTypescript(`:${node.content.namespacedName}`)} {
            return {
              ${node.content.fields
                        .map((field) => {
                        if (field.optional) {
         ...
         * @private
         */
        _writeMessage: function(${printIf(isEmpty, "_")}msg${printIfTypescript(`: ${`PartialDeep<${node.content.namespacedName}>`}`)}, writer${printIfTypescript(`: protoscript.BinaryWriter`)})${printIfTypescript(`: protoscript.BinaryWriter`)} {
          ${node.content.fields
                    .map((field) => {
                    let res = "";
        ...
        `;
                if (isEmpty) {
                    result += `_readMessage: function(_msg${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}, _reader${printIfTypescript(`: protoscript.BinaryReader`)})${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}{
            return _msg;`;
                }
                else {
                    result += `_readMessage: function(msg${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}, reader${printIfTypescript(`: protoscript.BinaryReader`)})${printIfTypescript(`: ${`${node.content.namespacedName}`}`)}{
...
export function generate(fileDescriptorProto, identifierTable, options, plugins) {
    ...
    const jsonSerializers = !config.typescript.emitDeclarationOnly
        ...
        : "";
    const hasWellKnownTypeImports = imports.some(({ moduleName }) => moduleName === "protoscript");
    return `\
...
${printIf(hasSerializer || hasWellKnownTypeImports, `import * as protoscript from "protoscript";`)}
